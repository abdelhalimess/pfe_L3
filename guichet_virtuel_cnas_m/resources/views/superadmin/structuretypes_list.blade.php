...
<span>Add, edit and delete a structuretype</span>
@endsection
    ...
    <div class="modal fade" id="add-structuretype-modal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Add a structuretype type</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <input type="text" :class="[errors.name ? 'form-control form-control-danger' : 'form-control form-control-success']" placeholder="Enter structuretype name..." maxlength="25" v-model="newstructureTypeName" required v-on:input="errors.name=null" />
                    <p class="text-danger m-t-5" v-if="errors.name">@{{errors.name.toString()}}</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default waves-effect" data-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-primary waves-effect waves-light" v-on:click="add_structuretype()">Save</button>
                </div>
            </div>
        </div>
    </div>    
        <div class="modal fade" id="edit-structuretype-modal" tabindex="-1" role="dialog">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Edit structuretype</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <input type="text" :class="[errors.name ? 'form-control form-control-danger' : 'form-control form-control-success']" maxlength="25" required v-on:input="errors.name=null" :placeholder="selectedstructureTypeName" v-model="structureTypeName" />
                        <p class="text-danger m-t-5" v-if="errors.name">@{{errors.name.toString()}}</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default waves-effect" data-dismiss="modal">Cancel</button>
                        <button type="button" class="btn btn-primary waves-effect waves-light" v-on:click="update_structuretype(structureTypeName,selectedStructureTypeIndex)">Save</button>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            v-on:click="selectedStructureTypeIndex = index">
                                ...
                                <td>
                                    <div class="text-center">
                                        ...
                                        <i class="feather icon-trash text-danger f-18 clickable" v-on:click="deleteStructureType(structuretype.id, index)"
...
const app = new Vue({
  ...
  data() {
        ...
        methods:{
            ...
            //     axios.put('/structuretypes/' + this.selectedStructure, {
            //             'name': name,
            //         })
            //         .then(function(response) {
